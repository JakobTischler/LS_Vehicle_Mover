...
;(function($){
	$(document).ready(function() {
		...
		//FORM DATA RESTORE via SISYPHUS
		$('.info').show().each(function() {
			$(this).css({ marginLeft: $(this).width()/-2 });
		}).hide();
		$('form').sisyphus({
			onSave: function() {
				$('.info.save')
					.css({ background: '#FF9', marginLeft:'-127.5px' })
					.fadeIn(100)
					.fadeTo(2000, 1)
					.fadeOut(200);
			},
		...
		$('button').on('click', function(event) {
			...
			$('#origPos')
				...
				.attr({
					'data-title': vehNameTitle,
					'data-vehicleName': vehNameTitle,
					'title': "X: "+origAvg.xRounded+" / Z: "+origAvg.zRounded,
					'data-origPosX': origAvg.x,
					'data-origPosY': origAvg.y,
					'data-origPosZ': origAvg.z
				})
				.on('click', function() {
					var e = $(this),
						...
						e.addClass('isCloned')
							...
							.draggable({
								...
								drag: function(event, ui) {
									...
									newE.attr({
										'title': 'X: '+xRounded + ' // Z: ' + zRounded,
										'data-newPosX': newPosItemX,
										'data-newPosZ': newPosItemZ
									});
